...
def object_hash(fd, fmt, repo=None):
    ...
    if fmt == b'commit':
        obj = structures.Commit(repo, data)
    elif fmt == b'tree':
        obj = structures.Tree(repo, data)
    elif fmt == b'tag':
        obj = structures.Tag(repo, data)
    elif fmt == b'blob':
        obj = structures.Blob(repo, data)
    else:
...
def set_work_dir(repo, curr, sha):
    obj = object_read(repo, sha)
    if type(obj) == structures.Blob:
        with open(curr, 'w') as file:
            file.write(obj.blobdata.decode())
        return
    for record in obj.items:
        set_work_dir(repo, os.path.join(curr, record.path.decode()), record.sha.decode())
